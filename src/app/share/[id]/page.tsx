  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Firestore Timestamp or raw Date string
  ...
  if (!data || isExpired) {
    return (
      <main className="w-full min-h-screen flex flex-col items-center justify-center bg-slate-950 px-6 text-center">
        ...
        <p className="text-gray-400 mb-10 text-sm md:text-base break-keep max-w-xs leading-relaxed opacity-80 font-serif italic">
          "운명의 메시지는 24시간 동안만 머물다 시공간 너머로 흩어집니다. 당신의 운명을 지금 바로 확인해 보세요."
        </p>
        <button
          onClick={() => router.push('/')}
          className="px-12 py-4 bg-emerald-600 text-white font-bold rounded-full shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:scale-105 active:scale-95 transition-all tracking-widest"
        >
          나의 운명 확인하기 ✨
        </button>
      </main>
  ...
  return (
    <main className="w-full min-h-screen bg-slate-950 text-white font-sans overflow-x-hidden relative flex flex-col items-center pb-32">
      ...
      <div className="text-center py-20 border-t border-white/5 opacity-40 w-full max-w-4xl mt-12">
         <p className="text-xs tracking-widest uppercase mb-2 font-light">Hocis-Healing Mystic Tarot</p>
